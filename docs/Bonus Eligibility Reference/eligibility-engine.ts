...
function handleFirstTimeOnly(
  ...
  if (userCardsForIssuer.length > 0) {
    return {
      ...
      confidenceLevel: issuer.confidenceLevel as 'high' | 'medium' | 'low',
      greyAreaNotes: issuer.notes || undefined,
    };
  }
  return {
    cardId: card.id,
...
function handleNewToBank(
  ...
  const latestCancellation = userCardsForIssuer
    ...
    .sort(
      (a, b) => new Date(b.cancellationDate!).getTime() - new Date(a.cancellationDate!).getTime(),
    )[0];
  ...
  if (latestCancellation && issuer.exclusionPeriodMonths) {
    const cancellationDate = new Date(latestCancellation.cancellationDate!);
...
function handleOncePerCard(
  ...
  if (!latestCard.cancellationDate) {
    return {
      cardId: card.id,
      card,
      status: 'not_eligible',
...
function handleTimeBased(
  ...
  const cancellationDate = new Date(latestRelevant.cancellationDate!);
  const monthsSinceCancellation = differenceInMonths(now, cancellationDate);
