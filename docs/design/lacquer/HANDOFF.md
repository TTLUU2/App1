...
> Rehydrated from the original download bundle (2026-08-20). Bundle
> was `~/Downloads/design_handoff_lacquer_refresh/` and disappeared
> between the shared read and the copy; content below is the full
> README of that bundle, captured verbatim from the conversation.
> The 280KB visual reference `Brand & UI Direction.dc.html` was not
> captured — re-add it here if it resurfaces, alongside the runtime
> shim (`support.js`) it needs to render.
...
- `Brand & UI Direction.dc.html` — the design reference. Approved sections: `6a`, `5a`, `4a`, `3b`. Ignore `1a`, `2a`, `2b`, `2c`, `3a`. **Not currently in-repo** — re-add if you have it.
